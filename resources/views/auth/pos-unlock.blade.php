...
<head>
    ...
    <title>دخول نقطة البيع - {{ config('app.name') }}</title>
...
<body class="min-h-screen bg-gradient-to-br from-emerald-900 via-emerald-800 to-emerald-950 flex items-center justify-center p-4">
    <div class="w-full max-w-xs">
        <div class="flex flex-col items-center mb-6 text-center">
            ...
            <p class="text-emerald-200 text-sm mt-1">دخول نقطة البيع</p>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-2xl p-6">
            ...
            <form method="POST" action="{{ route('pos.unlock.attempt') }}" id="pin-form">
                @csrf
        ...
        <a href="{{ route('login') }}" class="block text-center text-emerald-200 text-sm mt-4 hover:text-white">
            دخول كمدير (اسم مستخدم وكلمة سر)
        </a>
    </div>
